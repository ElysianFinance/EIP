...
The `protocolDAO` needs funds to pay for regular protocol upgrades and keeper functionality. This EIP proposes to divert `1,000` SNX weekly to the protocolDAO which can then convert it to Ether (approximately `3.5 ETH` p/w at time of writing) to pay for maintenance.
...
The `sDAO` has been paying for upgrades and maintenance since inception of the protocol. Once the Elysian treasury moves to the Treasury Council ([EIP-155](./eip-155.md)), having to request funds constantly for protocol maintenance through the Council will become more onerous.

The main need is for Ether to deploy and connect new iterations of the protocol on both L1 and L2. A secondary need is to maintain the current `DebtCache` keepers on both layers. The latter is too complex to be run by the Chainlink Keeper network (currently operating on the fee pool closing keeper) and is already marked as deprecated with [EIP-156](./eip-156.md). As such, a future ECCP will be planned to reduce this weekly amount, once the `DebtCache` keeper has been eliminated.
...
As with other inflation diversion, this EIP requires an entry in `RewardsDistribution` for the protocolDAO to receive `1,000` each week of inflation.
